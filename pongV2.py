...
parser.add_argument('--state_size', default=8, help='Number of state values')
...
def extract_coords(preprocessed):
    my_paddle_row = np.flatnonzero(preprocessed[:,-9])
    opp_paddle_row = np.flatnonzero(preprocessed[:,9])
...
def calculate_ball_velocity(curr_frame, prev_frame):
    if prev_frame is None:
        return np.array(curr_frame+(0, 0))
    v_x = curr_frame[4] - prev_frame[4]
    v_y = curr_frame[5] - prev_frame[5]
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(int(args.epochs)):
            ...
            for episode in range(int(args.num_episodes)):
                ...
                img = env.reset()
                # Initialize empty position for first velocity calculation
                previous_state = None
                ...
                while True:
                    ...
                    state = calculate_ball_velocity(coords, previous_state)
